...
typedef W16 half_t;
typedef W32 word_t;
typedef W64s squad_t;

#define fatal(x) assert(0);
#define fatal2(x, y) assert(0);
...
#define BOUND_POS(N)		((int)(std::min(std::max(0LL, (N)), 2147483647LL)))
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  if (nsets <= 0)
    fatal2("cache size (in sets) `%d' must be non-zero", nsets);
  if ((nsets & (nsets-1)) != 0)
    fatal2("cache size (in sets) `%d' is not a power of two", nsets);
  /* blocks must be at least one datum large, i.e., 8 bytes for SS */
  if (bsize < 8)
    fatal2("cache block size (in bytes) `%d' must be 8 or greater", bsize);
  if ((bsize & (bsize-1)) != 0)
    fatal2("cache block size (in bytes) `%d' must be a power of two", bsize);
  if (usize < 0)
    fatal2("user data size (in bytes) `%d' must be a positive value", usize);
  if (assoc <= 0)
    fatal2("cache associativity `%d' must be non-zero and positive", assoc);
  if ((assoc & (assoc-1)) != 0)
    fatal2("cache associativity `%d' must be a power of two", assoc);
  if (!blk_access_fn)
...
cache_char2policy(char c)		/* replacement policy as a char */
{
  ...
  case 'f': return FIFO;
  default: fatal2("bogus replacement policy, `%c'", c);
  }
}
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if ((nbytes & (nbytes-1)) != 0 || (addr & (nbytes-1)) != 0)
    fatal2("cache: access error: bad size or alignment, addr 0x%08x", addr);

  /* access must fit in cache block */
  /* FIXME:
     ((addr + (nbytes - 1)) > ((addr & ~cp->blk_mask) + (cp->bsize - 1))) */
  if ((addr + nbytes) > ((addr & ~cp->blk_mask) + cp->bsize))
    fatal2("cache: access error: access spans block, addr 0x%08x", addr);

  /* permissions are checked on cache misses */

  /* check for a fast hit: access to same block */
  if ((CACHE_TAGSET(cp, addr) == cp->last_tagset) && (cp->last_blk != NULL))
    {
  ...
  case Random:
    ...
    break;
  default:
    fatal("bogus replacement policy");
  }
  ...
  if (repl->status & CACHE_BLK_VALID)
	...
	*repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
      ...
      cp->bus_free = std::max(cp->bus_free, (now + lat)) + 1;
 ...
 cache_hit: /* slow hit handler */
  ...
  return (int) std::max((squad_t)cp->hit_latency, (squad_t)(blk->ready - now));
...
void
cache_mark_all_ready(struct cache_t *cp)
{
  int i;
  struct cache_blk_t *blk;
