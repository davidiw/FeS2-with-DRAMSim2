...
public:
  // Constructors
  Sequencer(NodeID id, MessageBuffer* request_queue_ptr, CacheMemory* primary_data_cache_ptr, CacheMemory* primary_insn_cache_ptr);
  ...
  bool empty() const { return m_outstanding_count == 0; }
...
private:
  ...
  void hitCallback(const CacheMsg& request, DataBlock& data);
  ...
  MessageBuffer* m_request_queue_ptr;
  CacheMemory* m_primary_data_cache_ptr;
  CacheMemory* m_primary_insn_cache_ptr;
