...
{
  return *(g_system_ptr->getSequencer(node));
}

NodeID map_address_to_node(const Address& address)
{
  return DirectoryMemory::mapAddressToHomeNode(address);
}
...
NodeID closest_clockwise_distance(NodeID this_node, NodeID next_node)
{
  if (this_node <= next_node) {
    ...
    return (next_node - this_node + g_param_ptr->NUM_NODES());
  }
}
