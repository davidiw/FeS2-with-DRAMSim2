...
Func::Func(string id, const Location& location, 
  ...
  : Symbol(id, location, pairs, machine_ptr)
{ 
  m_type_ptr = type_ptr; 
...
void Func::funcDefinition(string& code) const
{
  if (isExternal()) {
    // Do nothing
  } else {
    ...
    if (getMachine() == NULL) {
      error("Function declarations currently must be in a state machine");
    }
    code += " " + getMachine()->toString() + "_Controller::" + cIdent() + "(";
