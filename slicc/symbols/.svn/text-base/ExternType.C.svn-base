...
ExternType::ExternType(string id, const Location& location, 
                 const Map<string, string>& pairs, 
                 const StateMachine* machine_ptr)
  : Type(id, location, pairs, machine_ptr)
{
  addPair("external", "yes");
}

string ExternType::methodId(string name, 
...
bool ExternType::methodAdd(string name, 
                           ...
                           Map<string, string>& pairs)
{
  ...
  if (methodExist(id)) {
    ...
    m_method_pairs_map.add(id, pairs);
    return true;
  }  
}
