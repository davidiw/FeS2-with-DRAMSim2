...
Type* MemberExprAST::generate(string& code) const
{
  ...
  Struct* struct_ptr = dynamic_cast<Struct*>(type_ptr); // Ok
  if (struct_ptr == NULL) {
    error("Invalid object field: Type '" + type_ptr->toString() + "' does not have any data members");
  }

  // Verify that this is a valid field name for this type
  if (!struct_ptr->dataMemberExist(*m_field_ptr)) {
    error("Invalid object field: Type '" + struct_ptr->toString() + "' does not have data member " + *m_field_ptr);
  }

  // Return the type of the field
  return struct_ptr->dataMemberType(*m_field_ptr);
}
