...
TypeFieldMemberAST::TypeFieldMemberAST(TypeAST* type_ast_ptr, string* field_id_ptr, PairListAST* pairs_ptr)
...
void TypeFieldMemberAST::generate(Struct* struct_ptr)
{
  // Lookup type
  Type* field_type_ptr = m_type_ast_ptr->lookupType();

  // Add data member to the parent type
  if (!struct_ptr->dataMemberAdd(*m_field_id_ptr, field_type_ptr, getPairs())) {
    error("Duplicate data member: " + struct_ptr->toString() + ":" + *m_field_id_ptr);
  }
}
