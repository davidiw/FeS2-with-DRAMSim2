...
void ActionDeclAST::generate()
{
  ...
  if (m_statement_list_ptr != NULL) {
    ...
    g_sym_table.newTempSym(new Var("address", getLocation(), type_ptr, "addr", getPairs()));
  ...
  if (machine_ptr == NULL) {
    ...
    machine_ptr->addAction(new Action(*m_ident_ptr, resource_list, getLocation(), getPairs(), machine_ptr));
  }
