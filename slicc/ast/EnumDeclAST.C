...
EnumDeclAST::EnumDeclAST(TypeAST* type_ast_ptr, 
                         ...
                         Vector<TypeFieldEnumAST*>* field_vec_ptr)
  : DeclAST(pairs_ptr)
{
...
void EnumDeclAST::generate()
{
  ...
  // Make the new type
  Enum* new_enum_ptr = new Enum(id, getLocation(), getPairs(), 
                                g_sym_table.getStateMachine());
  g_sym_table.newSym(new_enum_ptr);

  // Add all of the fields of the type to it
  if (m_field_vec_ptr != NULL) {
    int size = m_field_vec_ptr->size();
    for(int i=0; i<size; i++) {
      (*m_field_vec_ptr)[i]->generate(new_enum_ptr);
    }
  }
}
