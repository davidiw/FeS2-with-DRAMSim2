...
StructDeclAST::StructDeclAST(TypeAST* type_ast_ptr, 
...
void StructDeclAST::generate()
{
  string machine_name;
  string id = m_type_ast_ptr->toString();

  // Make the new type
  Struct* new_type_ptr = new Struct(id, getLocation(), getPairs(), 
...
void StructDeclAST::print(ostream& out) const 
{ 
  out << "[StructDecl: " << m_type_ast_ptr->toString() << "]"; 
}
