...
%union {
  ...
  // TypeFields
  TypeFieldMemberAST* type_field_member_ptr;
  Vector<TypeFieldMemberAST*>* type_field_member_vector_ptr;

  TypeFieldMethodAST* type_field_method_ptr;
  Vector<TypeFieldMethodAST*>* type_field_method_vector_ptr;

  TypeFieldEnumAST* type_field_enum_ptr;
  Vector<TypeFieldEnumAST*>* type_field_enum_vector_ptr;
...
%type <type_field_member_vector_ptr> type_members 
%type <type_field_method_vector_ptr> type_methods
%type <type_field_enum_vector_ptr> type_enums 
%type <type_field_member_ptr> type_member
%type <type_field_method_ptr> type_method
%type <type_field_enum_ptr> type_enum
...
%token <str_ptr> IDENT STRING NUMBER FLOAT LONG LIT_BOOL VOID
%token <str_ptr> IMBED IMBED_TYPE 
%token ASSIGN DOT SEMICOLON COLON
%token MACHINE_DECL IN_PORT_DECL OUT_PORT_DECL
%token PEEK ENQUEUE COPY_HEAD CHECK_ALLOCATE
%token DEBUG_EXPR_TOKEN DEBUG_MSG_TOKEN
%token ACTION_DECL TRANSITION_DECL TYPE_DECL STRUCT_DECL EXTERN_TYPE_DECL ENUM_DECL PARAMETER_DECL
...
decl:  MACHINE_DECL     '(' ident pair_list ')' '{' decl_list '}' { $$ = new MachineAST($3, $4, $7); }
    ...
    |  IN_PORT_DECL     '(' ident ',' type ',' expr pair_list ')' statement_list { $$ = new InPortDeclAST($3, $5, $7, $8, $10); }
    |  OUT_PORT_DECL    '(' ident ',' type ',' expr pair_list ')' SEMICOLON { $$ = new OutPortDeclAST($3, $5, $7, $8); }
    ...
    |  EXTERN_TYPE_DECL '(' type pair_list ')' SEMICOLON            { $$ = new ExternTypeDeclAST($3, $4, NULL); }
    |  EXTERN_TYPE_DECL '(' type pair_list ')' '{' type_methods '}' { $$ = new ExternTypeDeclAST($3, $4, $7); }
    |  STRUCT_DECL      '(' type pair_list ')' '{' type_members '}' { $$ = new StructDeclAST($3, $4, $7); }
    |  ENUM_DECL        '(' type pair_list ')' '{' type_enums   '}' { $$ = new EnumDeclAST($3, $4, $7); }
    ...
    |  PARAMETER_DECL '(' type ident ',' expr pair_list ')' SEMICOLON { $$ = new ParameterDeclAST($3, $4, $6, $7); }
    ;

// Type fields

type_members: type_member type_members { $2->insertAtTop($1); $$ = $2; }
            | { $$ = new Vector<TypeFieldMemberAST*>; }
            ;

type_member: type ident pair_list SEMICOLON { $$ = new TypeFieldMemberAST($1, $2, $3); }
           ;

// Methods
type_methods: type_method type_methods { $2->insertAtTop($1); $$ = $2; }
            | { $$ = new Vector<TypeFieldMethodAST*>; }
            ;
...
statement:  expr SEMICOLON { $$ = new ExprStatementAST($1); }
         ...
         |  ENQUEUE      '(' var ',' type ',' expr pair_list ')' statement_list { $$ = new EnqueueStatementAST($3, $5, $7, $8, $10); }
         |  PEEK         '(' var ',' type ')' statement_list { $$ = new PeekStatementAST($3, $5, $7, "peek"); }
         |  COPY_HEAD    '(' var ',' var ',' expr pair_list ')' SEMICOLON { $$ = new CopyHeadStatementAST($3, $5, $7, $8); }
...
literal: STRING   { $$ = new LiteralExprAST($1, "string"); }
       ...
       | LONG     { $$ = new LiteralExprAST($1, "int64"); }
       | FLOAT    { $$ = new LiteralExprAST($1, "double"); }
